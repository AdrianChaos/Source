...
void MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		else if (!_stricmp(szAttrName, MICTOK_TYPE))
		{
			...
			else if (!_stricmp(szAttrValue, "avatar")) pNewDesc->m_nType = MMIT_AVATAR;
			else _ASSERT(0);
		}
		...
		if (pNewDesc->m_nType == MMIT_AVATAR) {
			pNewDesc->m_bIsAvatar = true;
			pNewDesc->m_szAvatarHead[0] = 0;
			pNewDesc->m_szAvatarChest[0] = 0;
			pNewDesc->m_szAvatarHands[0] = 0;
			pNewDesc->m_szAvatarLegs[0] = 0;
			pNewDesc->m_szAvatarFeet[0] = 0;
			pNewDesc->m_nAvatarParts = 0;

			for (int i = 0; i < nAttrCount; i++) {
				element.GetAttribute(i, szAttrName, szAttrValue);
				if (!_stricmp(szAttrName, "avatar_head")) {
					strcpy_safe(pNewDesc->m_szAvatarHead, szAttrValue);
					if (szAttrValue[0]) pNewDesc->m_nAvatarParts |= (1 << MMCIP_HEAD);
				}
				else if (!_stricmp(szAttrName, "avatar_chest")) {
					strcpy_safe(pNewDesc->m_szAvatarChest, szAttrValue);
					if (szAttrValue[0]) pNewDesc->m_nAvatarParts |= (1 << MMCIP_CHEST);
				}
				else if (!_stricmp(szAttrName, "avatar_hand")) {
					strcpy_safe(pNewDesc->m_szAvatarHands, szAttrValue);
					if (szAttrValue[0]) pNewDesc->m_nAvatarParts |= (1 << MMCIP_HANDS);
				}
				else if (!_stricmp(szAttrName, "avatar_legs")) {
					strcpy_safe(pNewDesc->m_szAvatarLegs, szAttrValue);
					if (szAttrValue[0]) pNewDesc->m_nAvatarParts |= (1 << MMCIP_LEGS);
				}
				else if (!_stricmp(szAttrName, "avatar_feet")) {
					strcpy_safe(pNewDesc->m_szAvatarFeet, szAttrValue);
					if (szAttrValue[0]) pNewDesc->m_nAvatarParts |= (1 << MMCIP_FEET);
				}
			}
		}
		else {
			pNewDesc->m_bIsAvatar = false;
			pNewDesc->m_szAvatarHead[0] = 0;
			pNewDesc->m_szAvatarChest[0] = 0;
			pNewDesc->m_szAvatarHands[0] = 0;
			pNewDesc->m_szAvatarLegs[0] = 0;
